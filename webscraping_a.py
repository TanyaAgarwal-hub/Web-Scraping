...
Original file is located at
    https://colab.research.google.com/drive/1pUX1yQk-zwP-E_8C8jMCLTBMKuASPwHO
"""
...
def get_skill(skills):
  ...
  #   div = sp.select('div.Skills_chipContainer__mlLa7 > span')
  #   span = list()
  #   for d in div:
  #     span.append(d.text.upper())
  #   skills.append(span)
  # print(skills)
...
def get_salary(salary):
  ...
  #   resp = requests.get(link, headers=header)
  #   sp = BeautifulSoup(resp.content, 'html.parser')
  #   # div = sp.select('div.job-overview_jobDetails__kBakg div.job-overview_detailContainer__TpXMD')


  #   div = sp.find('div', {'data-cy' : 'payDetails'})
  #   if(div is None):
  #     span = 'Salary Missing'
  #   else:
...
# add skills - store page link of job and derive skills
...
# create pd dataframe of samples for 5 pages
...
def get_pd(page_till):
  ...
  for i in range(1, page_till+1):
    ...
    # details = dict()
    # details['Title'] = get_title(rows)
    # details['Location'] = get_location(rows)
    # details['Employment Type'] = get_type(rows)


    # details['Skills'] = get_skill(get_link(rows))
    obj = get_sp_links(get_link(rows))
    sk = get_skill(obj[0])
    ...
    # jobs = pd.concat([jobs, df], ignore_index=True)
...
# plt.pie(your_data.values(), labels=your_data.keys())
...
# box plot location vs skills &&&& hue = salary(below 100, 100-200, 200 above)
#-------------error---------------------------
# out = get_pd(5)

# # skills, location, salary
# len_loc, len_sal = len(out[1]), len(out[2])
# loc, sal = list(out[1].values()), list(out[2].values())
# if(len_loc > len_sal):
#   for _ in range(len_loc - len_sal):
#     sal.append(0)
# else:
#   for _ in range(len_sal - len_loc):
#     loc.append(0)
...
# plt.title('Location vs salary for various skills')
# plt.legend(out[0].keys())

# plt.xlabel('Role location')
# plt.xlabel('Role salary')
